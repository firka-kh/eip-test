...
## Актуализация от 26.03.2026

- **Формирование черновика договора**: изменен порядок секций в форме — данные представителя Грантодателя (ГРАНТДИҲАНДА) теперь заполняются в первую очередь.
- **Шаблон договора**: в `contract.md` уточнена структура блока подписей (полный набор реквизитов представителя).
...
- Порядок полей и групп фиксирован под шаблон договора (первым идет блок намояндаи ваколатдор).
