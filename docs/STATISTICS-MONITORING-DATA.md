...
- Визит 1 (анкета запуска): распределение по `score` (0-7) и итоговой оценке (`Успешно` ≥4, `Частичный эффект` 2-3, `Без эффекта` 0-1)
- Визит 1: доля выбравших каждый вариант использования средств (`fundsUsed`) и выполненных активностей (`activities`)
- Визит 2 (анкета устойчивости): распределение по `score` (0-7) и итоговой оценке (`Успешно` ≥6, `Частичный эффект` 3-5, `Без эффекта` 0-2)
- Визит 2: доля по статусу бизнеса (`businessStatus`), влиянию гранта (`grantImpact`), изменению дохода (`incomeChange`)
- Визит 2: распределение по диапазону среднего дохода (`incomeRange`) — статистика без баллов
- Визиты 3-4: состояние оборудования: `in_stock`, `not_used`, `sold`
- Визиты 3-4: состояние бизнеса: `active`, `suspended`, `closed`
...
- Визит 1: диапазон итогового балла (`score`, 0-7) и итоговая оценка
- Визит 1: использование средств (`fundsUsed`), выполненные активности (`activities`)
- Визит 2: диапазон итогового балла (`score`, 0-7) и итоговая оценка
- Визит 2: статус бизнеса (`businessStatus`), влияние гранта (`grantImpact`), изменение дохода (`incomeChange`), диапазон дохода (`incomeRange`)
- Визиты 3-4: состояние оборудования (`in_stock`, `not_used`, `sold`)
- Визиты 3-4: состояние бизнеса (`active`, `suspended`, `closed`)
...
- возвращает метрики по визитам, баллам первого визита и рискам
