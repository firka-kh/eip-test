...
- После решения Комитета заявка получает статус `approved`, но грант остаётся неактивным до подтверждения получения в разделе `Финансы`. Только после подтверждения получения гранта формируется график мониторинга и у Фасилитатора появляется саб-фильтр `Мониторинг`.
...
stateDiagram-v2
    ...
    approved --> approved: Финансы подтверждают получение гранта
    approved --> approved: Автоформирование мониторинга (4 визита) после подтверждения получения
    approved --> [*]: Мониторинг: готов к визиту / наблюдению
...
| `com_review` | `approved` | Комитет | Решение "Одобрить" (грант не активен до подтверждения получения) |
...
| `approved` | `approved` | Финансы | Подтверждение получения гранта; после этого активируется мониторинг и появляется список Фасилитатора |
