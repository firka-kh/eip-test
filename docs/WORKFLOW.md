...
- После решения Комитета заявка автоматически получает статус `approved` и запускает мониторинг: формируется 4 визита, а в интерфейсе появляется отдельный саб-фильтр `Мониторинг` для Фасилитатора.
...
stateDiagram-v2
    ...
    approved --> approved: Автоформирование мониторинга (4 визита)
