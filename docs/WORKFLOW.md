...
| Одобрена Комитетом | `approved` + `grantActive = false` | Финансы | Решение Комитета зафиксировано, но грант еще не активен; мониторинг недоступен |
| Грант активен | `approved` + `grantActive = true` | Фасилитатор / Финансы | Финансы подтвердили получение; доступен мониторинг |
...
stateDiagram-v2
    ...
    approved --> approved: Финансы подтверждают получение (grantActive = true)
    approved --> approved: Фасилитатор загружает подписанный договор
    approved --> [*]: Мониторинг после активации (4 визита)
...
| `approved` + `grantActive=false` | `approved` + `grantActive=true` | Финансы | Действие «Подтвердить получение гранта»; создается график мониторинга |
