...
- **Кнопка «Новая заявка»**: скрыта для роли «Финансы (Молия)» и доступна только для роли «Фасилитатор».
- **Договор гранта**: убрана кнопка «Сохранить черновик» в форме договора для избежания путаницы.
- **Интерфейс Фасилитатора**: кнопки «Рӯйхатро кушодан / Открыть список» и «Дархостҳоро кушодан / Открыть заявки» теперь отображаются исключительно во вкладке «Одобрено Комитетом».

## Актуализация от 26.03.2026

- **Реорганизация формы договора**: секция «ГРАНТДИҲАНДА (намояндаи ваколатдор)» перемещена в самое начало формы для удобства заполнения.
- **Обновление подписей в контракте**: в шаблоне `contract.md` уточнена формулировка заголовка «ГРАНТДИҲАНДА (намояндаи ваколатдор)» и структура полей (ФИО, Должность, Адрес, Телефон, E-mail).
...
Структура формы:
- Секция `ГРАНТДИҲАНДА (намояндаи ваколатдор)`.
- Секция `Шапка договора`.
- Секция `Стороны для преамбулы`.
- Секция `Раздел III. Маблағгузорӣ`.
- Секция `Раздел V. Банковские реквизиты`.
- Секция `Раздел VII. Имзоҳои тарафҳо`.
...
- Кнопки действий: `Сбросить автополя`, `Предпросмотр`, `Печать`, `Экспорт PDF`, `Бекор кардан / Отмена`.
- Кнопка создания новой заявки (`Дархости нав / Новая заявка`) и форма создания отображаются **только** при активной вкладке Фасилитатора.
...
## Раздел «Молия / Финансы» (`#approved-registry-block`)

Этот раздел предназначен для работы финансового отдела и мониторинга выплат.

### Структура
Раздел использует комбинированное отображение:
- Карточки утвержденных протоколов (`approved_list`).
- Карточки/строки одобренных заявок (`approved_item`).

### Основные элементы UI
- **Summary-плашка**: Показывает общее количество одобренных заявок и общую сумму к выплате.
- **Кнопка «Содирот ба CSV / Экспорт в CSV»**: Генерирует выгрузку по текущему отфильтрованному списку.
- **Фильтры**: Год, Месяц, Сектор, Регион, Пол и текстовый поиск (ID/ФИО).

### Индикаторы в карточке
- **Бейдж «Договор» / «Без договора»**: Позволяет визуально отличить заявки с загруженным подписанным сканом.
- **Бейдж протокола**: Зеленый бейдж с номером `СП-XXXX`.
- **Статус-маркер «Полностью завершена»**: Появляется только при наличии загруженного договора.
...
┌──────────────────────────────┐
│ Визит 1 (+30 дней)           │
│ Статус: 🔵 active — анкета запуска бизнеса │
│  ┌──────────────────────┐    │
│  │ 1. Использование средств │
│  │ 2. Что уже сделано       │
│  │ 3. Начал ли давать доход │
│  │ 4. Планы продолжать      │
│  │ [Сохранить] → Балл X/7   │
│  └──────────────────────┘    │
│ Визит 2 (+90 дней) — анкета устойчивости │
│  ┌──────────────────────┐    │
│  │ 1. Работает ли бизнес сейчас │
│  │ 2. Как грант повлиял на бизнес│
│  │ 3. Изменение дохода после гранта │
│  │ 4. Средний доход (без баллов) │
│  │ 5. Планы на 12 месяцев   │
│  │ [Сохранить] → Балл X/7   │
│  └──────────────────────┘    │
│ Визит 3 (+180 дней)          │
│ Статус: 🔴 pending           │
...
### Обязательные поля визита 1 (анкета запуска, бальная система)

Визит 1 (+30 дней) использует отдельную короткую анкету без вопросов о доходе и фото:

| Поле | Имя (name) | Баллы |
|------|-----------|-------|
| Использование средств гранта | `funds-{visitId}` | 0-2 |
| Что уже сделано (можно несколько) | `act-{visitId}` | 0-2 (сумма ограничена) |
| Начал ли бизнес приносить доход | `income-{visitId}` | 0-2 |
| Планы продолжать бизнес | `continue-{visitId}` | 0-1 |

Итоговый балл = сумма всех четырёх компонентов (максимум 7). Порог «Успешно (запуск)» — `score >= 4`, «Частичный эффект» — 2-3, «Без эффекта» — 0-1.
Если хотя бы один обязательный вопрос не заполнен → warning toast: `'Лутфан ҳамаи майдонҳои ҳатмиро пур кунед!'`

### Обязательные поля визита 2 (анкета устойчивости бизнеса, бальная система)

Визит 2 (+90 дней) заменён на 5-вопросную бальную анкету устойчивости:

| Поле | Имя (name) | Баллы |
|------|-----------|-------|
| Работает ли бизнес сейчас | `biz2-{visitId}` | 0-2 |
| Как грант повлиял на бизнес | `impact-{visitId}` | 0-2 |
| Изменился ли доход после гранта | `incchange-{visitId}` | 0-2 |
| Средний ежемесячный доход (только для статистики) | `incrange-{visitId}` | без баллов |
| Планы продолжать/развивать бизнес (12 мес.) | `continue2-{visitId}` | 0-1 |

Итоговый балл = сумма В1+В2+В3+В5 (вопрос о среднем доходе баллов не даёт). Максимум — 7. Порог «Успешно» — `score >= 6`, «Частичный эффект» — 3-5, «Без эффекта» — 0-2.
Поля «Другое» (`biz2-note-{visitId}`, `impact-note-{visitId}`, `incchange-note-{visitId}`) — текстовые пояснения, отображаются только при выборе соответствующего варианта.
Если хотя бы один обязательный вопрос не заполнен → warning toast: `'Лутфан ҳамаи майдонҳои ҳатмиро пур кунед!'`

### Обязательные поля визитов 3-4 (устойчивость бизнеса, стандартная анкета)

| Поле | ID | Описание |
|------|----|----------|
| Оборудование | `eq-{visitId}` | Состояние: в наличии / не используется / продано |
| Бизнес | `biz-{visitId}` | Статус: активен / приостановлен / закрыт |

Поля `Средний доход` и `Фото` убраны из анкеты мониторинга.
