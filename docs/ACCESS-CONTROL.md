...
*(Финансовый отдел имеет доступ 👁 Просмотр только к `approved` заявкам со статусом «Полностью завершена»)*
...
| Действие | Функция в коде | Условие |
|----------|---------------|---------|
| Просмотр реестра | через фильтр `Молия / Финансы` | Строго заявки `approved` с прикрепленным договором |
