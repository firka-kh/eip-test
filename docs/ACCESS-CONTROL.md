...
graph LR
    subgraph "Роли"
        ...
        FIN["💰 Финансовый отдел<br/>(finance)"]
    end
...
*(Финансовый отдел имеет доступ 👁 Просмотр ко всем одобренным заявкам и протоколам)*
...
Режим `Молия / Финансы` использует общую базу одобренных Комитетом заявок и протоколов. Подписанный договор в карточке отображается как признак завершенности, но не является обязательным условием для видимости в этом режиме.
...
| Действие | Функция в коде | Условие |
|----------|---------------|---------|
| Просмотр данных | через фильтр `Молия / Финансы` | Все одобренные заявки (`approved`) и утвержденные протоколы |
| Экспорт выписки | кнопка `Содирот ба CSV` | Выгрузка полей: ID, ФИО, сектор, сумма, протокол, договор |
| Скачивание договора | `downloadCurrentGrantAgreementFromModal()` | В карточке заявки в режиме просмотра |
...
| Подтвердить получение гранта | Заявка имеет `status = approved` и еще не активирована |
| Создать мониторинг (4 визита) | Только после подтверждения финансистом (`grantActive = true`) |
...
flowchart TD
    ...
    B -->|finance| FINC{Заявка<br/>завершена?}
